...
def resize_leftside(img, df):
  # 왼위, 왼아래, 오위, 오아래
    for idx in range(len(df)):
        lux = df['왼옆면_왼위_x'][idx]
        luy = df['왼옆면_왼위_y'][idx]
        ldx = df['왼옆면_왼아래_x'][idx]
        ldy = df['왼옆면_왼아래_y'][idx]
        rux = df['왼옆면_오위_x'][idx]
        ruy = df['왼옆면_오위_y'][idx]
        rdx = df['왼옆면_오아래_x'][idx]
        rdy = df['왼옆면_오아래_y'][idx]
...
def make_final_csv(): # real_lr : 1(오른쪽), 0(왼쪽)
  final_df = pd.DataFrame(columns=['id', 'front_thick_width','side_thick_width', 'real_lr'])
